 ...
 */
export function createTokenPoolManager(tokenPool) {
  let _tokenRrIndex = 0;
  ...
  function getNextToken() {
    ...
    // Prefer tokens without auth errors and not in cooldown
    const healthy = candidates.filter(c => !c.inCooldown && !c.hasAuthError);
    ...
    for (const t of tierOrder) {
      ...
      if (t === "available" || t === "unknown") {
        // Round-robin within healthy bucket
