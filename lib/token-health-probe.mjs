...
export function createTokenHealthProbe({
  ...
  const _lastProbeAt = new Map(); // tokenName -> unix ms (for adaptive intervals)
      ...
      const req = doRequest(url, { method: "POST", headers }, (res) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode === 200) {
            ...
            healthManager?.reportHealResult(tokenEntry.name, true);
            resolve({ status: "verified", latencyMs, statusCode: 200 });
          } else if (res.statusCode === 401 || res.statusCode === 403) {
            // 401 = expired, 403 = revoked — both need token refresh
            const label = res.statusCode === 403 ? "revoked" : "expired";
            markTokenAuthError?.(tokenEntry.name);
            tokenRefresher?.handleAuthError(tokenEntry).then(r => {
              log(`[TokenProbe] ${res.statusCode} (${label}) on ${tokenEntry.name} — refresh ${r.refreshed ? "OK" : "FAILED"}`);
              if (r.refreshed) clearTokenAuthError?.(tokenEntry.name);
            }).catch(() => {});
            // Set cooldown so routing avoids this token while refreshing
            setTokenCooldown?.(tokenEntry, 60_000, `probe_${res.statusCode}_auth`);
            healthManager?.reportError(tokenEntry.name, res.statusCode, "auth_error");
            healthManager?.reportHealResult(tokenEntry.name, false);
            resolve({ status: "auth_error", latencyMs, statusCode: res.statusCode, body: body.slice(0, 200) });
          ...
          } else if (res.statusCode === 500 || res.statusCode === 529) {
            healthManager?.reportError(tokenEntry.name, res.statusCode, "server_error");
            healthManager?.reportHealResult(tokenEntry.name, false);
            resolve({ status: "server_error", latencyMs, statusCode: res.statusCode, body: body.slice(0, 200) });
          } else {
  ...
  async function probeAll() {
    ...
    for (const entry of tokenPool) {
      // Adaptive probing: skip tokens that were probed recently (based on their health state interval)
      const lastProbe = _lastProbeAt.get(entry.name) || 0;
      const probeInterval = healthManager ? healthManager.getProbeInterval(entry.name) : intervalMs;
      if (lastProbe > 0 && now - lastProbe < probeInterval) {
        continue; // not due yet
      }
      _lastProbeAt.set(entry.name, now);
      ...
      try {
        ...
        log(`[TokenProbe] ${entry.name}: ${result.status} (${result.latencyMs}ms)${healthManager ? ` [health=${healthManager.getHealthState(entry.name)}]` : ""}`);
      } catch (err) {
        _results.set(entry.name, {
          status: "probe_error",
          error: err.message,
  ...
  function start() {
    ...
    // Use 30s check interval with adaptive per-token probing
    const checkInterval = healthManager ? Math.min(intervalMs, 30_000) : intervalMs;
    _startupTimer = setTimeout(() => {
      _startupTimer = null;
      probeAll();
      _timer = setInterval(probeAll, checkInterval);
    }, 30_000);
  }
