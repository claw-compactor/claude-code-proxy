      ...
      if (apiRes.statusCode !== 200) {
        ...
        apiRes.on("end", () => {
          ...
          if ((apiRes.statusCode === 500 || apiRes.statusCode === 529) && !retried) {
            ...
            retried = true;
            log(`[${ts()}] STREAM_${apiRes.statusCode}_RETRY reqId=${reqId} token=${tokenEntry.name} — backoff 2s then retry`);
            eventLog.push("retry", { reqId, model, source, reason: "server_error", status: apiRes.statusCode, delay: 2000 });
            setTimeout(() => {
              streamFromAnthropicDirect(body, model, reqId + "-retry", source, res, release, tokenEntry, cacheCtx);
            }, 2000);
            return;
      ...
      if (apiRes.statusCode !== 200) {
        ...
        apiRes.on("end", () => {
          ...
          if ((apiRes.statusCode === 500 || apiRes.statusCode === 529) && !retried) {
            healthManager?.reportError(tokenEntry.name, apiRes.statusCode, "server_error");
            retried = true;
            log(`[${ts()}] NATIVE_STREAM_${apiRes.statusCode}_RETRY reqId=${reqId} token=${tokenEntry.name} — backoff 2s then retry`);
            eventLog.push("retry", { reqId, model: rawModel, source, reason: "server_error", status: apiRes.statusCode, delay: 2000 });
            setTimeout(() => {
              streamAnthropicNative(body, reqId + "-retry", source, res, release, tokenEntry, cacheCtx);
            }, 2000);
            return;
