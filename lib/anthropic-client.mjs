      ...
      if (apiRes.statusCode !== 200) {
        ...
        apiRes.on("end", () => {
          ...
          if (apiRes.statusCode === 401) {
            ...
            // 401 with no content sent → refresh token and retry once
            tokenRefresher.handleAuthError(tokenEntry).then(result => {
              if (result.refreshed && !retried) {
                retried = true;
                log(`[${ts()}] STREAM_401_RETRY reqId=${reqId} token=${tokenEntry.name} — retrying with refreshed token`);
                streamFromAnthropicDirect(body, model, reqId + "-retry", source, res, release, tokenEntry, cacheCtx);
                return;
              }
              log(`[${ts()}] ANTHROPIC_401 reqId=${reqId} token=${tokenEntry.name} refreshed=${result.refreshed} retried=${retried}`);
              eventLog.push("error", { reqId, mode: "anthropic_direct", model, source, status: 401 });
              safeWrite(sseChunk(reqId, `[Anthropic API error: HTTP 401 — auth expired]`));
              ...
              safeWrite(sseChunk(reqId, `[Anthropic API error: HTTP 401]`));
              safeWrite(sseFinishChunk(reqId, "stop"));
              safeWrite("data: [DONE]\n\n");
              safeEnd();
              doRelease();
      ...
      const apiReq = httpsRequest(url, { method: "POST", headers: hdrs }, (apiRes) => {
        ...
        apiRes.on("end", () => {
          ...
          if (apiRes.statusCode === 401) {
            ...
            tokenRefresher.handleAuthError(tokenEntry).then(result => {
              const err = new Error(`Anthropic 401: ${resBody.slice(0, 200)}`);
              err.statusCode = 401;
    ...
    const apiReq = httpsRequest(url, { method: "POST", headers }, (apiRes) => {
      ...
      if (apiRes.statusCode !== 200) {
        ...
        apiRes.on("end", () => {
          ...
          if (apiRes.statusCode === 401 && !retried) {
            recordWorkerError(tokenEntry.name, "auth_expired", `401: ${errBody.slice(0, 100)}`);
            tokenRefresher.handleAuthError(tokenEntry).then(result => {
              if (result.refreshed) {
                retried = true;
              ...
              safeWrite(errBody || JSON.stringify({ type: "error", error: { type: "authentication_error", message: "Unauthorized" } }));
              safeEnd();
              doRelease();
            }).catch(() => {
              res.writeHead(401, { "content-type": "application/json" });
    ...
    return new Promise((resolve, reject) => {
      ...
      const apiReq = httpsRequest(url, { method: "POST", headers: hdrs }, (apiRes) => {
        ...
        apiRes.on("end", () => {
          ...
          if (apiRes.statusCode === 401) {
            recordWorkerError(tokenEntry.name, "auth_expired", `401`);
            tokenRefresher.handleAuthError(tokenEntry).then(result => {
              const err = new Error(`Anthropic 401`);
              err.statusCode = 401;
              err.refreshed = result.refreshed;
